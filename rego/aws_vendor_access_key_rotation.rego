...
# Convert the threshold from days to nanoseconds.
# Rego time functions work in nanoseconds, so we convert:
# days * 24 hours * 60 minutes * 60 seconds * 1,000,000,000 nanoseconds
threshold_ns := globals.vendor_key_max_age_days * 24 * 60 * 60 * 1000000000

# Get the current time in nanoseconds
now := time.now_ns()

# Helper: check if the IAM user has a tag with Key="type" and Value="vendor".
...
# Skip this rule if the IAM user is not tagged as a vendor.
...
# Determine if Access Key 1 is stale (older than threshold).
# This rule is true only when BOTH conditions inside are true (implicit AND):
#   1. The key is active
#   2. The time since last rotation exceeds the threshold
key1_stale if {
    key1_active
...
# the result is "fail" if EITHER key1 OR key2 is stale.
result = "fail" if {
    is_vendor
    key1_stale
}
...
# sprintf() formats a string with values from the input data.
currentConfiguration := sprintf("Type: vendor | AccessKey1Active: %s, LastRotated: %s | AccessKey2Active: %s, LastRotated: %s", [
...
expectedConfiguration := sprintf("Vendor access keys should be rotated at least every %d days.", [globals.vendor_key_max_age_days])

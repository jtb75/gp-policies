...
# We use the same 30-day threshold as human users since untagged
# accounts should be treated with the strictest policy.
...
# We also confirm the user does NOT have a recognized type tag,
# so this only fires for untagged or incorrectly tagged users.
result = "fail" if {
...
expectedConfiguration := sprintf("All IAM users should have a valid type tag (user/service/vendor). Untagged access keys must be rotated at least every %d days.", [globals.user_key_max_age_days])

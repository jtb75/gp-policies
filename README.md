...
This repository manages 27 CCRs across nine categories:
...
- **Tag Enforcement** — 7 rules requiring valid `type` tags on IAM users, specific role lists, consumer roles, deploy roles, and roles with external trust relationships
...
This runs all 94 fixture/rule combinations and reports pass/fail. When adding a new rule, add its test cases to `validate_fixtures.py` in the `TESTS` list.
